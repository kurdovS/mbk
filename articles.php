<?php
function class_articles($brand, $page)
{
	//01_подключаем все подключаемые файлы для articles
	...
	if(preg_match('/^[0-9]+?/', $page)){
		require_once($_SERVER['DOCUMENT_ROOT'] . '/includes/for_articles/articles/articles.inc');
		$articles = articles($brand, $page);
		$title = '<title>Статьи - MBK-Delivery</title>';
		$description = 'Статьи о еде | Страница ' . $page . ' | MBK-Delivery';
		$keywords = 'Макдональдс доставка, Бургер Кинг доставка, KFC доставка, Статьи о еде, Макдональдс доставка на дом, Бургер Кинг доставка на дом, KFC доставка на дом, mcdonalds доставка, burgerking доставка, kfc доставка';
		$css_js = '<link rel="stylesheet" href="/includes/for_articles/articles/articles.css" />
					<script src="/includes/for_articles/articles/articles.js"></script>';
		$in_body = '<div id="wrap">
<div id="articles_wrap">
	<a href="/' . $brand . '"><div id="back_to_main">&larr; На главную</div></a>
	<div id="articles_view_switcher">
		<h1>Статьи</h1>
		<div id="switcher_view">
			<div class="switcher_img" id="view_icons"><img src="/includes/for_articles/articles/icons_article/icons.png" /></div>
			<div class="switcher_img" id="view_rows"><img src="/includes/for_articles/articles/icons_article/rows.png" /></div>
		</div>
	</div>
	<div id="articles_area">' . $articles . '</div>
</div></div>';
	...
	//если запрошена конкретная статья
		...
		require_once($_SERVER['DOCUMENT_ROOT'] . '/includes/for_articles/article/article.inc');
		$css_js = '<link rel="stylesheet" href="/includes/for_articles/article/article.css" />';
		//подключение к БД
		...
		mysqli_close($db);
		$title = '<title>' . $art['title'] . ' - MBK-Delivery</title>';
		$description = $art['title'] . ' | MBK-Delivery';
		$keywords = 'Макдональдс доставка, Бургер Кинг доставка, KFC доставка, Статьи о еде, Макдональдс доставка на дом, Бургер Кинг доставка на дом, KFC доставка на дом, mcdonalds доставка, burgerking доставка, kfc доставка';

		$in_body = article($brand, $page);
	}
