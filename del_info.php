...
function del_info()
{
	...
	//получим из БД информацию о онформационных статьях
	$query = 'SELECT * FROM `info_pages`';
	$res = mysqli_query($db, $query);
	$unp_num = mysqli_num_rows($res);				//всего статей

	//определим больше ли 10 информационных статей
	if($unp_num > 10){
		$unp_max = 10;
	...
	for($i = 0; $i < $unp_max; $i++){
		...
		$html .= '
			<tr>
				...
				<td class="info_table_regular_td_l"><div class="del_info">Удалить</div></td>
			</tr>';
	}
